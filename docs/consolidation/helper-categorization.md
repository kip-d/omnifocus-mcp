...
**Created:** 2025-11-07
**Purpose:** Categorize all 52 helpers to guide Phase 2B refactoring
**Phase:** 2A.3 - Value Categorization
...
**Justification:** Complex logic functions may be worth keeping if used frequently. Simple wrappers should be evaluated for inlining.
...
**Size Impact:** ~540KB eliminated (30 scripts × 18KB helpers)
**Performance Impact:** 2-67x faster across 28-30 scripts
...
**Size Reduction:** ~200 LOC from helpers.ts + cleanup
**Maintenance Impact:** Single source of truth for each operation
...
**Size reduction:** ~364 LOC (~12KB)
**Risk:** ZERO (no usage means no breakage)
**Verification:** Run grep before deletion to confirm
...
**Target:** 156+ uses across 29 scripts
**Strategy:** Convert to pure OmniJS v3 (Phase 1 proven pattern)
...
**Size reduction:** ~200 LOC
**Risk:** LOW (keep the better implementation)
...
**Size reduction:** 28 scripts × 18KB = **504KB**
**Performance:** 10-100x faster per script
**Maintenance:** Simpler pure OmniJS code
...
This categorization enables Phase 2A.4 to identify specific consolidation opportunities and create detailed migration paths for each script category.
...
The most-used helper (`safeGet()` - 156 uses) causes the biggest performance penalty (50% slower). Converting away from it provides the single biggest optimization opportunity.
