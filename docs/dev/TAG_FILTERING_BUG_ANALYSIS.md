...
The tag filtering in the tasks query tool has a **critical implementation gap**: the tool layer accepts advanced tag
filters with operators (OR, AND, NOT_IN) but the script layer **ignores these operators and always uses AND logic**.
...
- **Why**: Script ignored the operator and applied AND logic to ["test", "mcp-test"], but the AND comparison logic has a
  bug
...
- The gap was likely introduced when the script was refactored to use OmniJS directly (see comment on line 2 of
  list-tasks.ts)
