...
## 🚨 CRITICAL: Integration Test Debugging - Uncaught Exception Pattern (November 2025)

**Date Discovered:** November 9, 2025  
**Context:** Script helper consolidation integration test failures  
**Impact:** Server crashes, timeouts, test suite failures

### The Problem

Integration tests using shared MCP server instances were experiencing mysterious timeouts:
- ✅ Read operations work perfectly
- ❌ Write operations hang and timeout (60 seconds)
- ❌ All subsequent operations timeout after first write failure
- ✅ End-to-end tests (fresh server per test) work fine

**Critical Symptom:** Server process crashes completely after first write operation error, making all subsequent requests impossible.

### Root Cause

**Uncaught exception in error handler crashes entire Node.js process:**

```typescript
// ❌ WRONG - src/tools/index.ts (lines 132-143)
} catch (error) {
...
1. OmniAutomation throws `OmniAutomationError` (e.g., script fails with code 1)
2. Tool.execute() propagates exception
3. tools/index.ts catch block LOGS then RE-THROWS
4. MCP SDK async handler doesn't catch the re-thrown error properly
5. Node.js uncaught async exception crashes entire process
6. No global error handlers to catch it

**Why read operations work but write operations don't:**
- OmniFocus permission/accessibility issues often affect writes more than reads
- Failed osascript returns exit code 1 for writes
- Server crashes before timeout can occur
- Fresh servers work because they start clean each time

### The Fix

**Two-part solution implemented:**

1. **Convert errors to McpError** (prevents crashes):
```typescript
// ✅ CORRECT - Convert to proper MCP error
} catch (error) {
  correlatedLogger.error(`Tool execution failed: ${name}`, {...});
  
  // Don't re-throw - convert to McpError
...
2. **Add defensive global error handlers** (safety net):
```typescript
// In src/index.ts
...
**Systematic approach that identified the issue:**

1. **Pattern Recognition**
   - End-to-end tests: 17/17 passing (fresh server per test)
   - MCP protocol tests: 3/7 passing (shared server)
   - Write operations hang after first occurrence

2. **Minimal Reproduction**
   ```javascript
   // Created test-write-isolation.js
   Step 1: Initialize → ✅ Works
   Step 2: Read Operation → ✅ Works  
   Step 3: Write Operation → ❌ SERVER CRASHED
   ```

3. **Error Trace Analysis**
   - Examined server logs for uncaught exceptions
   - Traced error propagation through call stack
   - Identified re-throw location in tools/index.ts

4. **Hypothesis Testing**
   - Confirmed: Fresh servers work (process starts clean)
   - Confirmed: Shared servers crash (uncaught exception)
   - Verified: Error handler re-throws instead of converting

### Lessons Learned

**Testing Patterns:**
- ⚠️ **Shared server tests expose server lifecycle issues** that per-test servers hide
- ✅ **Fresh server per test hides server crash bugs** - good for functionality, bad for reliability
- ✅ **Mixed approach** - Use both patterns to catch different issue types

**Error Handling:**
- ❌ **Never re-throw errors in async handlers** - converts to uncaught exceptions
- ✅ **Always convert to McpError** - maintains MCP protocol compliance
- ✅ **Add global error handlers** - defensive programming for production reliability
- ⚠️ **MCP SDK async handlers** don't catch re-thrown errors properly

**Debugging Process:**
- ✅ **Compare working vs failing scenarios** - End-to-end vs MCP protocol tests
- ✅ **Create minimal reproductions** - Isolate the exact failure point
- ✅ **Trace error propagation** - Follow the exception through the call stack
- ✅ **Test hypothesis systematically** - Fresh vs shared servers revealed the pattern

### Files Involved

- `src/tools/index.ts` - Error re-throw location (needs McpError conversion)
- `src/index.ts` - Missing global error handlers
- `src/omnifocus/OmniAutomation.ts` - Original error source
- `tests/integration/mcp-protocol.test.ts` - Failing tests (shared server)
- `tests/integration/tools/unified/end-to-end.test.ts` - Working tests (fresh servers)

### Prevention

**For future MCP server development:**

1. **Never re-throw errors in async tool handlers** - always convert to McpError
2. **Always add global error handlers** - process.on('uncaughtException') and 'unhandledRejection'
3. **Test with both shared and fresh servers** - catches different issue types
4. **Create minimal reproductions** - fastest path to root cause
5. **Trace error propagation** - don't assume MCP SDK catches everything

### Related Issues

- MCP Server Async Operation Lifecycle (this file, line 1150)
- Integration test patterns need both shared and per-test servers
- Error handling must convert to McpError, not re-throw
