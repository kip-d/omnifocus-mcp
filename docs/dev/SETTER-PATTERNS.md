...
| 6   | Tags (`Task`/`Project`)                                               | `Tag[]` relationship                   | OmniJS bridge | **Not a value setter.** `task.addTags()` / `task.tags = …` silently no-op in JXA — assign via OmniJS `addTag()` inside an `evaluateJavascript` block (`src/contracts/ast/mutation-script-builder.ts`, task tags ~640, project tags ~1004) | ❌ silent no-op (JXA)                             | n/a                                                    | n/a | n/a             | Yes (assignment silently fails) |
