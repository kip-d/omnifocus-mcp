...
| AppleEvent preflight fails but OmniFocus has since quit → `STATUS: SKIPPED`, exit 0 (not WEDGED)                                | The app can quit in the window between the liveness pgrep and the preflight timing out; a re-check before logging distinguishes "quit mid-probe" from "still running, not answering"                                                                               |
| Overlapping invocations (e.g. a manual run during the scheduled minute) can each create the item before seeing the other's      | Accepted: the next run updates one and WARNs about the extras by id — delete the extras by hand (see `findReviewItems`/the multi-item WARNING in `guided-review-push.ts`)                                                                                          |
| `list_for_review` and `pattern_analysis` are fetched **sequentially**, not concurrently                                         | Deliberate (OMN-320): both are whole-DB OmniJS scans (~14-16s each); OmniFocus's `evaluateJavascript` engine is not safely reentrant for two concurrent multi-second calls — a Promise.all attempt killed the first real scheduled run with a signal, not an error |
