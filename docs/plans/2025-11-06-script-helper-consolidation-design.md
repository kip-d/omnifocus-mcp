...
**Approach:** Three-phase iterative refinement

## Executive Summary

This design outlines a comprehensive refactoring of the OmniFocus MCP script and helper infrastructure. Through three iterative phases, we will consolidate 62 scripts down to ~55, aggressively refactor the helper layer, and re-optimize scripts with improved tooling. The unified 4-tool API remains unchanged.

**Key Insight:** Rather than attempting perfect planning upfront, we use empirical iteration - learn by doing, measure results, refine based on reality.
...
*Note: This is an estimate. Empirical iteration means we'll adjust based on what we learn.*
