...
| ---------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/tools/unified/compilers/QueryCompiler.ts`                         | Modify | Tasks-side merge semantics; export `STATUS_TO_PROJECT`, `extractTextCondition`; `CompiledQuery.projectFilter` field; projects branch in `compile()` |
| `src/tools/unified/compilers/filter-merge.ts`                          | Create | `mergeConflictChecked`, `deepEqual`, `INPUT_KEY_OF` reverse-map, `emptyOperatorError`, `conflictError`                                              |
