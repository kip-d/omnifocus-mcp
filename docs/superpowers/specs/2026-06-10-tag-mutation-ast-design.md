...
| `MoveTagNode`          | `moveTag(tag: Expr, position: TagMovePosition)` where `TagMovePosition = {kind:'root'} \| {kind:'underTag'; var}` | `moveTags([<tag>], tags.ending \| <var>)` — one node, three consumers (nest/unparent/reparent); root = `tags.ending`, see the §3 amendment (live API rejects null)                              |
