...
var __projectTarget_0 = (function () {
  ...
  return {
    ...
    allMatches: exact.map(function (p) {
      return { id: p.id.primaryKey, name: p.name };
    }),
  };
})();
```
...
```json
{
  "metadata": {
    "warnings": [
      "Multiple projects named \"Home Renovation\" found (3 total). Showing tasks from the first match. Use project ID to target a specific one."
    ],
    "duplicateProjects": [
      { "id": "abc123DEFgh", "name": "Home Renovation" },
      { "id": "xyz789GHIjk", "name": "Home Renovation" },
      { "id": "qrs456MNOpq", "name": "Home Renovation" }
    ]
  }
}
```

The `duplicateProjects` array gives the LLM assistant all matching project IDs so it can immediately present
disambiguation options to the user or make follow-up queries by ID — no extra round-trip needed. The human-readable
`warnings` string provides context; the structured array provides actionable data.
...
if (typeof __projectTarget_0 !== 'undefined' && __projectTarget_0 && __projectTarget_0.duplicates > 0) {
  __warnings.push("Multiple projects named \\"" + "${projectValue}" + "\\" found ("
    ...
    + "Use project ID to target a specific one.");
