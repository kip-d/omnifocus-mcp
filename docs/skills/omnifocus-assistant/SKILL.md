...
| "Process my inbox" — also read `references/task-creation.md` before the Execute step (batch-create has a silent `project` limitation) and `references/gtd-methodology.md` for the waiting-for / someday-maybe capture conventions (defer window and note format) the clarify step delegates to                                                | `references/workflow-process-inbox.md`  |
...
| "Guided review" / "what needs a decision?" / "quick review" — one item at a time over the existing detectors; a `quick` pass needs today's `manage_reviews.list_for_review` slice, so also read `references/workflow-weekly-review.md` steps 9–10 for the batch `mark_reviewed` envelope                                                      | `references/workflow-guided-review.md`  |
...
| Creating or rewriting a task or project — naming, time estimates, sequencing, **repetition / recurrence rules**, project **review intervals**; for **which tag to use**, also read `references/gtd-methodology.md` — the context-tag vocabulary lives there, and `task-creation.md` covers only nested-tag syntax and `tag_manage` mechanics  | `references/task-creation.md`           |
