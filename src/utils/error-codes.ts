...
/**
 * Get error metadata including recovery suggestions
 */
export function getErrorMetadata(code: ErrorCode): { recoverable: boolean; category: string } {
  // Determine if error is potentially recoverable with retry
  ...
  if ([ErrorCode.OMNIFOCUS_NOT_RUNNING, ErrorCode.PERMISSION_DENIED, ErrorCode.SCRIPT_TIMEOUT, ErrorCode.SCRIPT_ERROR, ErrorCode.EXECUTION_ERROR, ErrorCode.NULL_RESULT].includes(code)) {
    category = 'omnifocus';
  } else if ([ErrorCode.INVALID_OPERATION, ErrorCode.INVALID_PARAMS, ErrorCode.MISSING_PARAMETER, ErrorCode.INVALID_ID].includes(code)) {
    category = 'validation';
  } else if ([ErrorCode.CREATE_FAILED, ErrorCode.UPDATE_FAILED, ErrorCode.DELETE_FAILED, ErrorCode.SEARCH_ERROR].includes(code)) {
    category = 'operation';
  } else if ([ErrorCode.ANALYSIS_ERROR, ErrorCode.VELOCITY_ERROR].includes(code)) {
    category = 'analytics';
  } else if ([ErrorCode.INTERNAL_ERROR, ErrorCode.UNKNOWN_ERROR].includes(code)) {
    category = 'generic';
  }

  return {
    recoverable: recoverableErrors.has(code),
    category,
  };
}
