/**
 * Script to analyze recurring tasks in OmniFocus - V3 Pure OmniJS
 *
 * V3 Changes:
 * - Removed getUnifiedHelpers() dependency
 * - Direct try/catch instead of safeGet()
 * - OmniJS direct property access (no function calls)
 * - V3 response format
 * - Preserved fetchRepeatRuleViaBridge() for complex rule extraction
 *
 * Features:
    ...
    if (!allTasks) {
      return JSON.stringify({
        ...
        error: {
          ...
          operation: 'analyze_recurring_tasks'
        }
      });
    }
    const now = new Date();
