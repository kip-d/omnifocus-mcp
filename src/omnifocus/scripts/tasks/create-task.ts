 ...
 */
export function createCreateTaskScript(taskData: any): string {
  return `
  ...
  // Execute with function argument passing (NOT template substitution)
  return createTask(${JSON.stringify(taskData)});
  `;
}

/**
 * Script to create a new task in OmniFocus  
 * MINIMAL VERSION: Reduced script size by removing complex features to avoid JXA truncation
 */
export const CREATE_TASK_SCRIPT_MINIMAL = `
  ...
  (() => {
    ...
    try {
      ...
      // Add to inbox by default (keep simple)
      doc.inboxTasks.push(task);
      
      // Set dates if provided (basic handling)
      if (taskData.dueDate) {
        try {
          task.dueDate = new Date(taskData.dueDate);
        } catch (e) {
      ...
      return JSON.stringify({
        ...
        created: true,
        inInbox: true,
        note: 'Created with minimal script - advanced features not available'
      });
      
    } catch (error) {
      return formatError(error, 'create_task_minimal');
    }
  })();
`;

/**
 * Full-featured script (may hit size limits with complex parameters)
