/**
 * Ultra-fast optimized script for today's agenda - V3 Pure OmniJS
 *
 * V3 Changes:
 * - Removed getUnifiedHelpers() dependency
 * - Inline safeIsCompleted and isTaskEffectivelyCompleted logic
 * - V3 response format (was v1)
 * - Kept single-pass algorithm optimization
 */
export const TODAYS_AGENDA_SCRIPT = `
  // Helper function for safe completed check
  function safeIsCompleted(task) {
    // Check both task completion and parent project completion status
    try {
    ...
    try {
      ...
      if (project) {
        ...
        if (project.status && project.status() === 'done') return true;
      }
    } catch (e) {}
