...
export function analyzeReviewGaps(projects: Project[]): ReviewGapsResult {
  ...
  if (neverReviewed.length > 0) {
    recommendations.push(
      `${neverReviewed.length} project(s) have never been reviewed. Consider setting up review schedules.`,
    );
  }
  if (overdueProjects.length > 0) {
    recommendations.push(
      `${overdueProjects.length} project(s) are overdue for review. Schedule time for GTD weekly review.`,
    );
  }
