...
- includeStats: true (projects) attaches taskCounts {total, available, completed} and nextTask to every row (plus a stats block). These ride on includeStats, NOT on fields — they are attached even when a fields list omits them, so a fields projection combined with includeStats returns the requested fields PLUS these extras. taskCounts.total/completed count the root task's direct children; taskCounts.available counts actionable descendants at any depth (can exceed a direct-children count on projects with nested groups; 0 reliably means "no workable task"). Costs a whole-database task scan regardless of filter/limit — omit it when you only need project identity/metadata
