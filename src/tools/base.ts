...
export abstract class BaseTool<TSchema extends z.ZodType = z.ZodType, TResponse = unknown> {
  ...
  /**
   * Get JSON Schema for MCP tool advertisement.
   * All concrete tools MUST override this with a hand-crafted minimal schema.
   * Server-side validation still uses the full Zod schema.
   */
  get inputSchema(): Record<string, unknown> {
    throw new Error(
      `${this.constructor.name} must override inputSchema with a hand-crafted JSON Schema. ` +
        'See OmniFocusReadTool for an example.',
    );
  }
