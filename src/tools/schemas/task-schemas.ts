...
export const UpdateTaskSchema = z.object({
  ...
  responseLevel: z.enum(['full', 'standard', 'minimal', 'ultra'])
    .optional()
    .default('standard')
    .describe('Response detail level: "full" (all task data + metadata), "standard" (task data only), "minimal" (success + id + key changes), "ultra" (success + id only - 83% token reduction). Use "ultra" for maximum context savings in bulk operations.'),
});
