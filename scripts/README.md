...
| `ops/guided-review-push.ts`                          | Counts review decisions waiting (existing detectors) and creates/updates ONE OmniFocus inbox item, `Review: N decisions waiting` — silent when nothing is waiting (OMN-314). Manual run: `OF_MCP_REVIEW_ITEM_PREFIX="__TEST__ Review: " npx tsx scripts/ops/guided-review-push.ts <dev-checkout>/dist/index.js --mode quick` |
| `ops/of-mcp-guided-review`                           | The deployed wrapper the launchd job runs (PATH, pgrep fail-safe, bounded run; Mon–Sat 07:00, Saturday = deep mode)                                                                                                                                                                                                          |
| `ops/com.omnifocus-mcp.guided-review.plist.template` | launchd plist template the installer substitutes (also bakes in `OF_MCP_REVIEW_ITEM_PREFIX`)                                                                                                                                                                                                                                 |
| `ops/install-guided-review-schedule.sh`              | Install/reload the guided-review launchd job; `--verify` kickstarts it once and checks the log; `--uninstall` boots it out and removes the plist. See `docs/dev/guided-review-push.md`.                                                                                                                                      |
