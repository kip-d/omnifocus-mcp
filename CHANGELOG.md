...
- **Unified Helper System** - Consolidated helpers into single bundle
  ...
  - Migrated all 34 scripts across tasks, projects, folders, tags, analytics, reviews, recurring, export, and
    perspectives
...
- **11 Helper Functions** - Marked for removal in v2.3.0
  ...
  - **Functions**: `getAllHelpers`, `getAllHelpersWithBridge`, `getCoreHelpers`, `getMinimalHelpers`, `getBasicHelpers`,
    `getAnalyticsHelpers`, `getListHelpers`, `getFullStatusHelpers`, `getRecurrenceHelpers`,
    `getRecurrenceApplyHelpers`, `getTagHelpers`
...
- **Server Hang Bug** (commit 3b0be51)
  ...
  - Root cause: `getMinimalHelpers()` called `generateHelperConfig()` before `getCoreHelpers()`, which already includes
    it
...
- **🤖 Real LLM Testing Framework** (commits b784d1e, d3e8d74)
  ...
  - **Hardware Performance Documentation**: Empirical testing on M2 MacBook Air (24GB) shows 23-36s per test with
    phi3.5:3.8b
  - **Test Script Optimization**: Hardware-aware timeouts and fail-fast test scripts for different development
    environments
...
**Total Implementation**: ~18 hours across 6 major commits representing completion of Phase 1 Foundation, Phase 2 Quick
Optimizations, and Phase 3 High-Value Features from the improvement roadmap.
...
- **Schema Transform Functions**: Union types with `.transform()` for optional date fields (projectId, dueDate,
  deferDate, completionDate)
...
🚀 **Ready for Production**: This release represents a major reliability milestone with bulletproof async operations and
...
- **Complete v2.1.0 Architecture Migration**: Systematic elimination of unsafe `any` types
  ...
  - Added comprehensive schema validation using Zod (ListResultSchema, SimpleOperationResultSchema,
    AnalyticsResultSchema, etc.)
...
- **Tools Migrated** (18 total):
  ...
  - Analytics Suite: ProductivityStatsToolV2, TaskVelocityToolV2, OverdueAnalysisToolV2, WorkflowAnalysisTool,
    PatternAnalysisTool
...
- **Fixed "Can't convert types" JXA errors**: Resolved script size limit issues affecting project updates and folder
  deletion
...
- **Pattern Analysis Integration**: Merged pattern-analysis branch with comprehensive database analysis
  - Added PatternAnalysisToolV2 with 8 analysis patterns (duplicates, dormant projects, tag audit, deadline health,
    waiting tasks, estimation bias, next actions, review gaps)
...
This release represents a complete overhaul of the OmniFocus MCP server, achieving 95% performance improvements and
fixing all major JXA limitations through innovative bridge patterns.
...
Query tasks from any OmniFocus perspective WITHOUT changing the user's window! Access your custom perspectives and
built-in views programmatically while respecting the user's workflow.
...
This beta release adds full support for moving tasks between parents, projects, and the inbox using the
`evaluateJavascript()` bridge, fixing the third major JXA limitation!
...
The evaluateJavascript() bridge has now fixed THREE major JXA limitations that were previously considered impossible to
overcome!
...
This beta release adds full support for recurring tasks using the `evaluateJavascript()` bridge, overcoming another
significant JXA limitation.
...
This is an alpha release to gather feedback on the new architecture. The API may change based on user experience
testing.
...
Improved JavaScript filtering performance through elimination of redundant wrapper functions and timestamp-based
comparisons:
...
- For typical queries (50-100 results from 2000+ tasks), combined with v1.14.0's whose() removal, significant
  performance gains
...
The schema was incorrectly validating the `days` parameter even when it wasn't needed for overdue queries. Now properly
handles conditional parameters based on query type.
...
JXA's `whose({completed: false})` was identified as a performance bottleneck. Manual filtering proved significantly
...
This release completely reverts the hybrid architecture approach after v1.13.1 testing showed it still had unacceptable
performance.
...
This release introduces a hybrid JXA + Omni Automation approach using the `evaluateJavascript()` bridge for improved
performance.
