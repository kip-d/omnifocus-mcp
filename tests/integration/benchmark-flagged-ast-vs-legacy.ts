/**
 * Benchmark: Legacy flagged script vs AST-generated flagged query
 *
 * Compares performance and result parity between the OmniJS-first
 * flagged perspective script and the AST builder approach.
 *
 * Run: npx tsx tests/integration/benchmark-flagged-ast-vs-legacy.ts
 */
...
import { FLAGGED_TASKS_PERSPECTIVE_SCRIPT } from '../../src/omnifocus/scripts/tasks/flagged-tasks-perspective.js';
...
// --- Legacy approach: run the flagged script directly via osascript ---

function runLegacyFlagged(limit: number): { taskCount: number; timeMs: number; error?: string } {
  // The flagged script uses OmniJS template literals — needs JXA outer wrapper
  const script = FLAGGED_TASKS_PERSPECTIVE_SCRIPT.replace('{{limit}}', String(limit))
    .replace('{{includeCompleted}}', 'false')
    .replace('{{includeDetails}}', 'false');

  const tmpFile = join(tmpdir(), `benchmark-legacy-flagged-${Date.now()}.js`);
...
function runASTFlagged(limit: number): { taskCount: number; timeMs: number; scriptTimeMs?: number; error?: string } {
...
function main() {
  ...
  console.log('  Benchmark: Legacy Flagged Script vs AST Builder');
  console.log('='.repeat(70));
  ...
  // Legacy benchmark
  console.log('Running legacy flagged script (direct osascript)...');
  const legacyResult: BenchmarkResult = {
    approach: 'Legacy (OmniJS-first, template literals)',
    taskCount: 0,
    timings: [],
    minMs: Infinity,
    avgMs: 0,
  ...
  for (let i = 0; i < ITERATIONS; i++) {
    const r = runLegacyFlagged(LIMIT);
    console.log(
  ...
  // AST benchmark
  console.log('\nRunning AST flagged query (via MCP server)...');
  ...
  if (legacyResult.taskCount !== astResult.taskCount) {
    ...
    console.log('    (Minor differences expected: AST uses normalized filters, legacy uses raw OmniJS iteration)');
  } else {
