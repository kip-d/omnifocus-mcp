/**
 * Benchmark: Legacy upcoming script vs AST-generated upcoming query
 *
 * Compares performance and result parity between the JXA+helpers
 * upcoming script and the AST builder approach.
 *
 * Run: npx tsx tests/integration/benchmark-upcoming-ast-vs-legacy.ts
 */
...
import { GET_UPCOMING_TASKS_ULTRA_OPTIMIZED_SCRIPT } from '../../src/omnifocus/scripts/date-range-queries.js';
...
// --- Legacy approach: run the upcoming script directly via osascript ---
// Note: Legacy uses JXA + unified helpers (not OmniJS bridge) — osascript -l JavaScript works directly

function runLegacyUpcoming(days: number, limit: number): { taskCount: number; timeMs: number; error?: string } {
  const script = GET_UPCOMING_TASKS_ULTRA_OPTIMIZED_SCRIPT.replace('{{days}}', String(days))
    .replace('{{includeToday}}', 'true')
    .replace('{{limit}}', String(limit));

  const tmpFile = join(tmpdir(), `benchmark-legacy-upcoming-${Date.now()}.js`);
...
function runASTUpcoming(
  days: number,
  limit: number,
): { taskCount: number; timeMs: number; scriptTimeMs?: number; error?: string } {
...
function main() {
  ...
  console.log('  Benchmark: Legacy Upcoming Script vs AST Builder');
  console.log('='.repeat(70));
  console.log(`  Iterations: ${ITERATIONS}   Limit: ${LIMIT}   Days ahead: ${DAYS_AHEAD}`);
  console.log();
  ...
  // Legacy benchmark
  console.log('Running legacy upcoming script (direct osascript)...');
  const legacyResult: BenchmarkResult = {
    approach: 'Legacy (JXA + unified helpers)',
    taskCount: 0,
    timings: [],
    minMs: Infinity,
    avgMs: 0,
  ...
  for (let i = 0; i < ITERATIONS; i++) {
    const r = runLegacyUpcoming(DAYS_AHEAD, LIMIT);
    console.log(
  ...
  // AST benchmark
  console.log('\nRunning AST upcoming query (via MCP server)...');
  ...
  for (let i = 0; i < ITERATIONS; i++) {
    const r = runASTUpcoming(DAYS_AHEAD, LIMIT);
  ...
  if (legacyResult.taskCount !== astResult.taskCount) {
    ...
    console.log(
      '    (Minor differences expected: AST checks project completion status, legacy uses isTaskEffectivelyCompleted)',
    );
  } else {
