...
When('I count incomplete tasks', async function() {
  this.response = await this.callTool('get_task_count', {
    completed: false
  });
});

Then('I should have more than {int} incomplete tasks', async function(minCount) {
...
When('I get today\'s agenda', async function() {
  this.response = await this.callTool('todays_agenda', {});
});

Then('I should see my agenda for today', function() {
...
When('I create a simple test task', async function() {
...
Then('the task should be created', function() {
...
When('I list all my tags', async function() {
...
Then('I should see a list of tags', function() {
  ...
  if (this.response.tags.length > 0) {
    ...
    this.response.tags.slice(0, 5).forEach(tag => {
