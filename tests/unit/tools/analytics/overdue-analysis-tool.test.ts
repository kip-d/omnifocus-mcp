...
describe('OverdueAnalysisTool', () => {
  ...
  it('should return cached results when available', async () => {
    ...
    expect(res.data.summary.totalOverdue).toBe(5);
  });
  ...
  it('should return analytics response with overdue summary', async () => {
    ...
    expect(res.success).toBe(true);
    expect(res.data.summary.totalOverdue).toBe(15);
    expect(res.data.summary.overduePercentage).toBe(25.5);
    expect(res.data.overdueTasks.length).toBe(1);
    expect(res.data.patterns.length).toBe(2);
    expect(res.data.recommendations.length).toBe(2);
  ...
  it('should extract key findings from overdue data', async () => {
    ...
    // Should include total overdue in key findings
    expect(res.summary.key_findings.some((f: string) => f.includes('42'))).toBe(true);
  });
  ...
  it('should handle empty overdue results', async () => {
    ...
    const res: any = await tool.execute({} as any);
    expect(res.success).toBe(true);
    expect(res.data.summary.totalOverdue).toBe(0);
    expect(res.data.overdueTasks.length).toBe(0);
  });
  ...
  it('should support different groupBy options', async () => {
    ...
    // Test with groupBy: 'tag'
    const resTag: any = await tool.execute({ groupBy: 'tag' } as any);
    expect(resTag.success).toBe(true);

    // Test with groupBy: 'none'
    const resNone: any = await tool.execute({ groupBy: 'none' } as any);
