(() => {
  ...
  try {
    ...
    try {
      const jsCode = '(() => { const r = {}; if (typeof Perspective !== "undefined") { if (Perspective.all) { r.count = Perspective.all.length; r.names = Perspective.all.map(p => p.name); } } return JSON.stringify(r); })()';
